...
package com.mandacarubroker.elements.controllers.exceptions;
...
public class ControllerExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<StandardError> handleConstraintViolationException(ConstraintViolationException ex) {
        ...
        StandardError standardError = new StandardError(LocalDateTime.now(), HttpStatus.BAD_REQUEST, ex.getMessage());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(standardError);
    }
    @ExceptionHandler(ValidationException.class)
    public ResponseEntity<StandardError> validatorViolationException(ValidationException ex) {

        StandardError standardError = new StandardError(LocalDateTime.now(), HttpStatus.CONFLICT, ex.getMessage());
